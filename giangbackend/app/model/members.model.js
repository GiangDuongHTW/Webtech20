...
const Member = function(member) {
    this.Buchname = member.Buchname;
    this.Autor = member.Autor;
    this.Inhalt = member.Inhalt;
    this.Seite = member.Seite;
...
Member.updateById = (id, member, result) => {
    sql.query(
        "UPDATE members SET Buchname = ?, Autor = ?, Inhalt = ?, Seite = ?, Note = ? WHERE id = ?",
        [member.Buchname, member.Autor, member.Inhalt, member.Seite, member.Note, id],
        (err, res) => {
            if (err) {
                console.log("error: ", err);
                result(null, err);
